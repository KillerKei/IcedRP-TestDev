	...
	while true do
		...
		if payDst < 1 then
			if paycheckReady == true or  paycheckReady == false then
				irpCore.Game.Utils.DrawText3D(vector3(-1083.176, -248.0483, 37.76324), "Press [E] to pick up your paycheck!", 0.4)
			end
...
AddEventHandler('irp:restoreLoadout', function()
	local playerPed = PlayerPedId()
	local ammoTypes = {}

	RemoveAllPedWeapons(playerPed, true)

	for k,v in ipairs(irpCore.PlayerData.loadout) do
		...
		for k2,v2 in ipairs(v.components) do
			local componentHash = irpCore.GetWeaponComponent(weaponName, v2).hash
...
RegisterNetEvent('irp:addInventoryItem')
AddEventHandler('irp:addInventoryItem', function(item, count)
	for k,v in ipairs(irpCore.PlayerData.inventory) do
		if v.name == item.name then
			irpCore.PlayerData.inventory[k] = item
			break
		end
	end

	--irpCore.UI.ShowInventoryItemNotification(true, item, count)

	if irpCore.UI.Menu.IsOpen('default', 'irp-core', 'inventory') then
		irpCore.ShowInventory()
	end
end)

RegisterNetEvent('irp:removeInventoryItem')
AddEventHandler('irp:removeInventoryItem', function(item, count)
	for k,v in ipairs(irpCore.PlayerData.inventory) do
		if v.name == item.name then
			irpCore.PlayerData.inventory[k] = item
			break
	...
	--irpCore.UI.ShowInventoryItemNotification(false, item, count)
...
RegisterNetEvent('irp:addWeapon')
AddEventHandler('irp:addWeapon', function(weaponName, ammo)
	local playerPed  = PlayerPedId()
	local weaponHash = GetHashKey(weaponName)
...
RegisterNetEvent('irp:addWeaponComponent')
AddEventHandler('irp:addWeaponComponent', function(weaponName, weaponComponent)
	local playerPed  = PlayerPedId()
	local weaponHash = GetHashKey(weaponName)
	local componentHash = irpCore.GetWeaponComponent(weaponName, weaponComponent).hash

	GiveWeaponComponentToPed(playerPed, weaponHash, componentHash)
end)

RegisterNetEvent('irp:removeWeapon')
AddEventHandler('irp:removeWeapon', function(weaponName, ammo)
	local playerPed  = PlayerPedId()
	local weaponHash = GetHashKey(weaponName)
...
RegisterNetEvent('irp:removeWeaponComponent')
AddEventHandler('irp:removeWeaponComponent', function(weaponName, weaponComponent)
	local playerPed  = PlayerPedId()
	local weaponHash = GetHashKey(weaponName)
	local componentHash = irpCore.GetWeaponComponent(weaponName, weaponComponent).hash
...
RegisterNetEvent('irp:pickupWeapon')
AddEventHandler('irp:pickupWeapon', function(weaponPickup, weaponName, ammo)
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if loadoutChanged and isLoadoutLoaded then
			irpCore.PlayerData.loadout = loadout
			TriggerServerEvent('irp:updateLoadout', loadout)
		end
	end
end)
