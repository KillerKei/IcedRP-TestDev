...
irpCore.SavePlayer = function(xPlayer, cb)
	...
	for i=1, #xPlayer.inventory, 1 do
		if irpCore.LastPlayerData[xPlayer.source].items[xPlayer.inventory[i].name] ~= xPlayer.inventory[i].count then
			table.insert(asyncTasks, function(cb)
			...
			irpCore.LastPlayerData[xPlayer.source].items[xPlayer.inventory[i].name] = xPlayer.inventory[i].count
		end
	end
