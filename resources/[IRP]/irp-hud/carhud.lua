        ...
        if vehModel ~= nil and vehModel ~= 0 then
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                	local finished = exports["irp-taskbar"]:taskBar(2000,"Toggling Anchor")
...
RegisterNetEvent("irp-jobmanager:playerBecameJob")
AddEventHandler("irp-jobmanager:playerBecameJob", function(job, name)
...
RegisterNetEvent('irp-voice:focus:set')
AddEventHandler('irp-voice:focus:set', function(hasFocus, hasKeyboard, hasMouse)
...
RegisterNetEvent("irp-admin:currentDevmode")
AddEventHandler("irp-admin:currentDevmode", function(devmode)
    currentValues["dev"] = devmode
end)

RegisterNetEvent("irp-admin:currentDebug")
AddEventHandler("irp-admin:currentDebug", function(debugToggle)
...
AddEventHandler('irp-base:playerSessionStarted', function()
