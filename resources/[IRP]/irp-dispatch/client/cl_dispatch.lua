...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if (IsControlJustReleased(0, 289) and (irpCore.GetPlayerData().job.name == 'ambulance' or irpCore.GetPlayerData().job.name == 'police') and not exports["irp-ambulancejob"]:GetDeath()) then
...
AddEventHandler("civilian:alertPolice",function(basedistance,alertType,objPassed,isGunshot,isSpeeder)
    ...
    if alertType == "robberyhouse" and not irpCore.GetPlayerData().job.name == 'police' then
      AlertCheckRobbery2()
    end
end)
...
function AlertFight()
  ...
  TriggerEvent('irp-outlawalert:combatInProgress')
...
RegisterCommand('rob', function()
  TriggerServerEvent('t1ger_drugs:DrugSaleInProgress')
end, false)
