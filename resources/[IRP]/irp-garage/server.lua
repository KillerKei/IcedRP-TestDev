...
irpCore.RegisterServerCallback('irp-garage:getOwnedCars', function(source, cb)
	...
	if DontShowPoundCarsInGarage == true then
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job LIMIT 25', {
...
AddEventHandler('garages:CheckForSpawnVeh', function(veh_id, garageCost)
	...
	TriggerClientEvent('DoLongHudText', src, 'You\'ve been deducted $'..garageCost, 1)
	else
		TriggerClientEvent('DoLongHudText', src, 'Vehicle released', 1)
	end
	...
	TriggerClientEvent('DoLongHudText', src, 'You don\'t have enough money to retrieve your car', 2)
end
end)
...
AddEventHandler('garages:CheckForVeh', function()
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND state = @state', {['@owner'] = player, ['@state'] = state, ['@vehicle'] = vehicle}, function(result)
		...
		TriggerClientEvent('garages:StoreVehicle', src, plate)
			...
			TriggerClientEvent('DoLongHudText', src, 'Theres no car or its not yours', 2)
		end		
	end)
end)
