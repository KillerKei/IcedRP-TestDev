...
local garages = {
	...
	[30] =  { loc = {-1479.6,-666.556,28.4}, spawn = {-1479.6,-666.556,28.4}, garage = 'U' },
			...
			if row["house_poi"] ~= nil then
				...
				if backdooroutside["x"] ~= 0.0 then
					if #(vector3(backdooroutside["x"],backdooroutside["y"],backdooroutside["z"]-0.3) - plyCoords) < 3.0 then
						DrawMarker(20,backdooroutside["x"],backdooroutside["y"],backdooroutside["z"]-0.3,0,0,0,0,0,0,0.701,1.0001,0.3001,222 , 50, 50,11,0,0,0,0)
					end
				...
				if backdoorinside["x"] ~= 0.0 then
					if #(vector3(backdoorinside["x"],backdoorinside["y"],backdoorinside["z"]-0.3) - plyCoords) < 3.0 then
						DrawMarker(20,backdoorinside["x"],backdoorinside["y"],backdoorinside["z"]-0.3,0,0,0,0,0,0,0.701,1.0001,0.3001,222 , 50, 50,11,0,0,0,0)
					end
					...
					if #(vector3(storage["x"],storage["y"],storage["z"]-0.3) - plyCoords) < 1.5 then
						...
						if IsControlJustReleased(2, 38) then
							if row["house_owner_cid"] == exports["isPed"]:isPed("cid") then
								...
								TriggerEvent("DoLongHudText", "You are not allowed to open other's stashes.", 1)
							end
						end
					end
				end
...
local blips = {
	...
	{-1479.6, -666.556, 28.4, 'U' },
	{-208.41, 307.5, 96.95, 'Rooster Cab'},	
}
...
AddEventHandler('Garages:HouseRemoveVehicle', function(veh)
	...
	dntdelete = plate
	TriggerServerEvent('garage:addKeys', veh , plate)
...
irpCore.TriggerServerCallback('irp-garage:getOwnedCars', function(ownedCars)
	...
	if ((count >= (page*10)) and (count < ((page*10)+10))) then
		...
		elseif curGarage ~= current_used_garage and curGarage ~= "Any" and vehicle_state ~= "Out" then
			Menu.addButton(tostring(vehicleName), "OptionVehicle", 0, "Garage " .. curGarage, " Engine %:" .. irpCore.Math.Round(enginePercent,1) .. "", " Body %:" .. irpCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
		else
...
function OptionVehicle(vehID)
	...
	ClearMenu()
	if vehID == 0 then
		TriggerEvent('DoLongHudText', 'You dont have vehicle in this garage', 2)
...
function drawDoLongHudText(text)
	SetDoLongHudTextTextEntry("STRING")
	AddTextComponentString(text)
	DrawDoLongHudText(false, false)
end
...
function spawnJudgeCar(model,garage)
		...
		TriggerServerEvent('garage:addKeys', spawned_car , plate)
				...
				while garagesRunning do
					...
					if dist < 35.0 then
						...
						if myroomtype ~= 3 and gar == "House" then
							...
							DrawMarker(20,pos[1],pos[2],pos[3],0,0,0,0,0,0,0.701,1.0001,0.3001,222 , 50, 50,dist,0,0,0,0)
						end
					end
...
AddEventHandler('garages:StoreVehicle', function()
    ...
    if vehicle == 0 then
		...
		TriggerEvent('DoLongHudText', 'There is no vehicle to store', 2)
	...
	Citizen.CreateThread(function()		
		...
		if DoesEntityExist(caissei) then	
			if plate ~= platecaissei then					
				...
				TriggerServerEvent('garage:removeKeys', platecaissei)
...
AddEventHandler('garages:PlayerEnd', function(plate)
	Citizen.CreateThread(function()		
		...
		local platecaissei = GetVehicleNumberPlateText(caissei)
		if DoesEntityExist(caissei) then	
			TriggerServerEvent('garage:addKeys', caissei , plate)
			TriggerServerEvent("garages:CheckGarageForVeh")
		else
			TriggerEvent("DoLongHudText","No Vehicle",2)
		end   
