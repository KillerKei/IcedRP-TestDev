...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    refillVehicle()
    exports["irp-taskbar"]:taskBar(timer, "Refueling")
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
     ...
     if IsControlJustPressed(0, 29) then
       if seatbelt == false then
         seatbelt = true
         TriggerEvent('DoLongHudText', 'Seat Belt Enabled', 1)
     ...
     if (GetPedInVehicleSeat(car, -1) == ped) then
         ...
         if IsControlJustReleased(0, Keys['M']) then
             if cruiseIsOn then
                 TriggerEvent('DoLongHudText', 'Cruise Disabled', 1)
             else
                 TriggerEvent('DoLongHudText', 'Cruise Activated', 1)
             end
            ...
            if DoesEntityExist(veh) and IsEntityAVehicle(veh) and #(GetEntityCoords(veh) - GetEntityCoords(PlayerPedId())) < 5.0 then
                ...
                if IsControlJustPressed(1, 38) then
                    ...
                    elseif irpCore.GetPlayerData().money > costs then
                        ...
                        TriggerServerEvent("carfill:pay",costs)
                    else
