                ...
                if DeliveryCounter > 0 then
                    ...
                    if IsControlJustReleased(1,38) then
                        ...
                        if lastCook+GracePeriod < GetGameTimer() then
                            ...
                            TriggerEvent("DoLongHudText","You must wait to swap to deliveries! (" .. (lastCook+GracePeriod-GetGameTimer())/1000 .." seconds)", 2)
                        end
...
function SetDelivery(foodReq)
    if DeliveryCounter ~= 2 then
        if exports["irp-inventory"]:hasEnoughOfItem(foodReq,1) then
            TriggerServerEvent("inventory:removeItem",foodReq, 1)
...
AddEventHandler("taco:successStart", function()
    ...
    TriggerServerEvent("player:receiveItem","weedtaco", 1)
    while ActiveRun do
        Wait(1)
...
function AttemptDropOffTaco()
    if exports["irp-inventory"]:hasEnoughOfItem("weedtaco",1) then
        TriggerServerEvent("inventory:removeItem","weedtaco", 1)
        ...
        if exports["irp-inventory"]:hasEnoughOfItem("inkedmoneybag",1) then     
            TriggerServerEvent("inventory:removeItem","inkedmoneybag", 1)   
        ...
        elseif exports["irp-inventory"]:hasEnoughOfItem("markedbills",1) then     
            TriggerServerEvent("inventory:removeItem","markedbills", 1)   
        ...
        elseif exports["irp-inventory"]:hasEnoughOfItem("rollcash",rollcount) then     
            TriggerServerEvent("inventory:removeItem","rollcash", rollcount)   
        ...
        elseif exports["irp-inventory"]:hasEnoughOfItem("band",bandcount) then     
            TriggerServerEvent("inventory:removeItem","band", bandcount)   
...
AddEventHandler("weed:successStart", function()
    ...
    if not exports["irp-inventory"]:hasEnoughOfItem("weed4g",weedcost) then
...
function StartWeedRun()
    TriggerServerEvent("inventory:removeItem","weed4g", weedcost)
    TriggerServerEvent("player:receiveItem","weed12oz",1)
    ActiveRun = true
end
...
function AttemptDropOff()
    if exports["irp-inventory"]:hasEnoughOfItem("weed12oz",1) then
        ...
        TriggerServerEvent("player:receiveItem","band",math.ceil(10*value))
        TriggerServerEvent("inventory:removeItem","weed12oz",1)
        ClearBlips()
