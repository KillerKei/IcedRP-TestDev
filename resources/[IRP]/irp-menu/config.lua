...
rootMenuConfig =  {
    {
        ...
        subMenus = {"general:escort", "general:putinvehicle", "general:unseatnearest", "general:flipvehicle", "general:civcuff", "general:civuncuff", "general:keysgive",  "general:emotes", "general:askfortrain"  }
    },
    {
        id = "police-action",
        ...
        subMenus = {"general:escort", "police:putinvehicle", "police:unseatnearest", "police:hardcuff", "police:softcuff", "police:uncuff", "police:panic", "police:pdrevive", "cuffs:checkinventory", "police:frisk", "police:mdt"}
    },
    {
        id = "police-vehicle",
    ...
    },
    {
        ...
        subMenus = {"medic:revive", "medic:heal", "medic:bigheal", "general:escort", "general:putinvehicle", "general:unseatnearest", "general:checktargetstates", "police:panic" }
    },
    {
        id = "policeDeadA",
        displayName = "10-13A",
...
newSubMenus = {
    ...
    ['general:civcuff'] = {
        title = "Civ Cuff",
        icon = "#cuffs-cuff",
        functionName = "irp-handcuffs:cuffcheck"
    },
    ['general:civuncuff'] = {
        title = "Civ Uncuff",
        icon = "#cuffs-cuff",
        functionName = "irp-handcuffs:uncuff"
    },    
    ['animations:brave'] = {
        title = "Brave",
    ...
    ['police:frisk'] = {
        ...
        functionName = "irp-policefrisk:menuEvent"
    },
    ['police:pdrevive'] = {
        title = "Revive",
...
RegisterNetEvent('irp-handcuffs:cuffcheck')
AddEventHandler('irp-handcuffs:cuffcheck', function()
    irpCore.TriggerServerCallback('irp-menu:HasTheItems', function(result)
        if result then
            ...
            TriggerEvent('DoLongHudText', 'No Handcuffs', 2)
        end
    end)
end)

RegisterNetEvent('irp-handcuffs:uncuff')
AddEventHandler('irp-handcuffs:uncuff', function()
    irpCore.TriggerServerCallback('irp-menu:HasKey', function(result)
        if result then
            local target, distance = irpCore.Game.GetClosestPlayer()
            if distance ~= -1 and distance <= 3.0 then
                playerheading = GetEntityHeading(GetPlayerPed(-1))
...
AddEventHandler('FlipVehicle', function()
    ...
    if IsPedInAnyVehicle(ped, false) then vehicle = GetVehiclePedIsIn(ped, false) else vehicle = GetClosestVehicle(coords.x, coords.y, coords.z, 5.0, 0, 71) end
        ...
        exports["irp-taskbar"]:taskBar(10000, "Flipping Vehicle Over")
...
AddEventHandler('civputInVehicle', function()
    ...
    if target and distance < 3 then
        TriggerServerEvent('irp-interactions:putInVehicle', GetPlayerServerId(target))
	else
        TriggerEvent('DoLongHudText', 'There is no player(s) nearby!', 2)
	end
end)

RegisterNetEvent('irp-interactions:putInVehicle')
AddEventHandler('irp-interactions:putInVehicle', function()
    local playerPed = PlayerPedId()
...
AddEventHandler('civoutOfVehicle', function()
    local target, distance = irpCore.Game.GetClosestPlayer()
    if target and distance < 3 then
        TriggerServerEvent('irp-interactions:outOfVehicle', GetPlayerServerId(target))
	else
		TriggerEvent('DoLongHudText', 'There is no player(s) nearby!', 2)
	end
end)

RegisterNetEvent('irp-interactions:outOfVehicle')
AddEventHandler('irp-interactions:outOfVehicle', function()
    local playerPed = PlayerPedId()
...
RegisterCommand('scuba', function()
    ...
    if vehFront > 0 then
        if GetVehicleDoorAngleRatio(vehFront, 5) ~= 0.0 then
            ...
            exports["irp-taskbar"]:taskBar(4000, "Grabbing Scuba Gear")		
	  		    loadAnimDict('anim@narcotics@trash')
    		    TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',0.9, -8, 1900, 49, 3.0, 0, 0, 0)	  		
...
AddEventHandler("UseOxygenTank",function()
    ...
    TriggerEvent('DoLongHudText', 'Equipped Scuba Gear', 1)
end)
