...
function lockpicking:StartMinigame(pins,retEvent)
  ...
  Citizen.CreateThread(function(...)
    ...
    local doCont = true
    doWait = true
    irpCore.TriggerServerCallback('lockpicking:GetLockpickCount', function(count)
    ...
    if doCont then
      ...
      while self.IsPlaying and pinsLeft > 0 do
        ...
        if IsControlJustPressed(0,24) or IsDisabledControlJustPressed(0,24) then
          ...
          if inRange then pinsLeft = pinsLeft - 1
            ...
            TriggerEvent('DoLongHudText', "You failed to crack the lock.", 2)
          end
          templateA.text = pinsLeft
      ...
      if pinsLeft <= 0 then
        didWin = true
        TriggerEvent('DoLongHudText', 'You cracked the lock', 2)
      end
      self.IsPlaying = false

      if retEvent then
        TriggerEvent(retEvent, didWin)
      else
      ...
      TriggerEvent('DoLongHudText', 'You do not have any lockpicks', 2)
    end
  end)        
end
