...
AddEventHandler('message:inDistanceZone', function(somethingsomething, messagehueifh)
    ...
    TriggerClientEvent('chat:addMessage', somethingsomething, { template = '<div class="chat-message nonemergency"><b> {0}</b> {1}</div>', args = { "Phone Number: ",  msg } })
end)
...
AddEventHandler('message:tome', function(messagehueifh)
    ...
    local msg = first .. "-" .. second .. "-" .. third
    TriggerClientEvent('chat:addMessage', src, { template = '<div class="chat-message nonemergency"><b> {0}</b> {1}</div>', args = { "Phone Number: ",  msg } })
end)
...
RegisterServerEvent('irp:checkPhoneCount')
AddEventHandler('irp:checkPhoneCount', function()
	local _source = source
	local xPlayer = irpCore.GetPlayerFromId(_source)
	if xPlayer.getInventoryItem('phone').count >= 1 then
		TriggerClientEvent('irp:heHasPhone', _source)
    else 
        TriggerClientEvent('DoLongHudText', _source, 'You dont have a phone, Buy one at your local store', 2)
