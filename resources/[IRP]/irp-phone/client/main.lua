...
RegisterNUICallback('vehiclePayImpound', function(data)
  ...
  print('fuck this is for impound payement fuck you')
...
AddEventHandler("phone:Garage", function(vehs)
  ...
  for ind, value in pairs(vehs) do
    ...
    local engine = value.engine_damage
    local body = value.body_damage
    local model = jsonparse["model"]
    local fuel = value.fuel
    local repaytime = value.repaytime

    if engine ~= nil then
      local somethingfinal = engine / 10 
      finalengine = irpCore.Math.Round(somethingfinal, 0)
    end

    if body ~= nil then
      local somethingfinal = body / 10
      finalbody = irpCore.Math.Round(somethingfinal, 0)
    end
    ...
    if repaytime ~= nil then
      local somethingfinal = (repaytime / 1000) 
      repaytime = irpCore.Math.Round(somethingfinal, 0)
    end
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsDisabledControlJustReleased(1, 199) then
      TriggerServerEvent("irp:checkPhoneCount")
    end
  end
end)
...
RegisterNUICallback('newContactSubmit', function(data, cb)
  TriggerEvent('chat:addMessage', {template = '<div class="chat-message phonem"><b> {0}</b> {1}</div>', args = { "Service: ",'Contact Saved!' } })
...
AddEventHandler('Client:UpdateTweet', function(data, handle2)
    ...
    print(handle)
    print('this is the handler dude ', exports["isPed"]:isPed("twitterhandle"))
    currentTwats[#currentTwats+1] = tweet 
    print('this is handle shit dude',handle)
...
RegisterNUICallback('newTwatSubmit', function(data, cb)
  ...
  print('this is submti',myHandlerTwat)
    closeGui()
...
RegisterNetEvent("nl-phone:userTwitter")

AddEventHandler("nl-phone:userTwitter", function(user)
  userTwitter = user
  --print(user)
