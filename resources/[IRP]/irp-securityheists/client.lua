...
function DropItemPedBankCard()
    ...
    if myluck == 1 then
        TriggerServerEvent("player:receiveItem","inkedmoneybag",1)
    elseif myluck == 2 then
        TriggerServerEvent("player:receiveItem","rollcash",1)
    ...
    if not pickingup then
       ...
       -- Citizen.Trace("Doing Animation")
        ...
        while pickingup do
            ...
            if chance < 10 then
                TriggerEvent("alert:noPedCheck", "banktruck")
                TriggerServerEvent("player:receiveItem","band",math.random(length))
            end

            TriggerServerEvent("player:receiveItem","rollcash",math.random(length))
